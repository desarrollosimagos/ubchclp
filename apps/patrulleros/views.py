...
class PatrulleroDatosView(DetailView):
    model = Patrullero
    ...
    def get(self, request, *args, **kwargs):
        ...
        if cedula is not None:
            ...
            queryset_p = Patrullero.objects.all()
            queryset_f = queryset.filter(cedula=cedula)
            
            if queryset_f.exists():
            
                queryset_v = queryset_f.values('cedula', 'nombre', 'telefono', 'cod_cargo', 'cod_ubch', 'nom_ubch')
                ...
                queryset_p_f = queryset_p.filter(cedula_jefe=cedula)
                queryset_p_v = queryset_p_f.values('id', 'cedula', 'p_nombre', 's_nombre', 'p_apellido', 's_apellido', 'telefono', 'twitter', 'agregado', 'direccion').order_by('id')
                
                if queryset_p_v.exists():
                    dato = []
                    for value in queryset_p_v:
                        dato.append(value)
                
                    response_data['patrullero'] = dato
                else:
...
class DeletePatrullero(DeleteView):
    model = Patrullero

    def delete(self, request, *args, **kwargs):
        response_data = {}
        existe = Patrullero.objects.filter(id=self.kwargs['pk'])
        if existe.exists():
